...
public class SampleView extends CallStackView {
    ...
    public SampleView() {
        super();
        //getTimeGraphViewer().setStartFinishTime(0, 100);
    ...
    protected void buildEntryList(final ITmfTrace trace, final ITmfTrace parentTrace, final IProgressMonitor monitor) {
        // Get State System:
        AbstractCallStackAnalysis moduleToSSTree = getCallStackModule(trace);
        if (moduleToSSTree == null) {
            return;
        }
        ITmfStateSystem ss = moduleToSSTree.getStateSystem();
        ...
        traceEntry = new TraceEntry(trace.getName(), ss.getStartTime(), ss.getCurrentEndTime());
        ...
        levelEntryAux = new LevelEntry("level 0", 0, ss.getStartTime() + 10, ss.getCurrentEndTime() - 2); //fRoot.getProfileData().getStartTime(), fRoot.getProfileData().getEndTime() + 1);
        traceEntry.addChild(levelEntryAux);
        levelEntryMap.put(trace, levelEntryAux);

        // Creating a eventEntry
        eventEntryAux = new EventEntry("function 1", 37, ss.getStartTime() + 10, ss.getCurrentEndTime() - 2); //fRoot.getProfileData().getStartTime() + 1, fRoot.getProfileData().getEndTime());
